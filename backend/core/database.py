...
class DatabaseManager:
    ...
    def _init_database(self):
        ...
        with self._get_connection() as conn:
            ...
            # Ensure legacy DBs get the is_superadmin column
    ...
    def _seed_admin(self, conn):
        ...
        conn.execute(
            """INSERT INTO USERS (userId, username, email, hashedPassword, is_admin, is_superadmin, createdAt)
               VALUES (?, ?, ?, ?, 1, 1, ?)""",
            (admin_id, "admin", "admin@mail.com", hashed, now),
        )
        print("=" * 60)
    ...
    # User operations
    # ------------------------------------------------------------------
    def create_user(self, username: str, email: str, password: str, is_admin: bool = False, is_superadmin: bool = False) -> dict:
        ...
        with self._get_connection() as conn:
            conn.execute(
                """INSERT INTO USERS (userId, username, email, hashedPassword, is_admin, is_superadmin, createdAt)
                   VALUES (?, ?, ?, ?, ?, ?, ?)""",
                (user_id, username, email, hashed, int(is_admin), int(is_superadmin), now),
            )
