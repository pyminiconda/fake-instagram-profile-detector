...
MODELS_DIR = os.path.join(os.path.dirname(os.path.dirname(os.path.dirname(os.path.abspath(__file__)))), "ml_pipeline", "models")
...
class PredictionEngine:
        ...
        try:
            ...
            if self.explainer is None:
                ...
                if model_type in ("RandomForestClassifier", "XGBClassifier",
                    ...
                    # For non-tree models (SVC, LogisticRegression, etc.)
                    # KernelExplainer needs a representative background dataset.
                    # Generate a synthetic one from the scaler's training statistics.
                    background = self._build_background_data()
            ...
            # TreeExplainer may return a list (one per class) — take class 0 (genuine)
            if isinstance(sv, list):
                values = sv[0][0] if len(sv) > 1 else -sv[0]
            else:
                # For binary classifiers, take genuine class (0) column if 3D
                if sv.ndim == 3:
                    values = sv[0, :, 0]
                elif sv.ndim == 2:
                    values = -sv[0] # If only 1D output, it's for class 1, so negate it
                else:
                    values = -sv
    ...
    def _build_background_data(self, n_samples: int = 100) -> np.ndarray:
        """
        Build a synthetic background dataset for KernelExplainer from
        the scaler's stored training-set statistics (mean_ and scale_).

        This approximates the training distribution so SHAP can compute
        meaningful marginal contributions for each feature.
        """
        import shap

        if self.scaler is not None and hasattr(self.scaler, 'mean_'):
            rng = np.random.RandomState(42)
            # Generate samples ~ N(0, 1), then the scaler will have already
            # been applied, so we work in scaled space directly.
            background = rng.randn(n_samples, len(self.scaler.mean_))
            # Use shap.kmeans to summarise to a small number of clusters
            # for faster KernelExplainer computation.
            return shap.kmeans(background, 10)
        else:
            # Last resort: return a zero-mean background
